...
#define AML_CMD_A2EMSG_LEN(m) (sizeof(struct lmac_msg) + (m? m->param_len : 0))
...
#define CMD_PRINT(cmd) do { \
    if (cmd->id != ME_TRAFFIC_IND_REQ) { \
        if (!is_mdnsoffload_msg(cmd->mm_sub_id)) { \
            AML_INFO("cmd tkn[%d]  flags:%04x  cmd:%4d-%-24s - reqcfm(%4d-%-s)\n", \
               cmd->tkn, cmd->flags, cmd->id, cmd->id == MM_OTHER_REQ ? AML_MM_OTHER_CMD2STR(cmd) : AML_ID2STR(cmd->id), \
