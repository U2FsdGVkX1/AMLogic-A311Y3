                                         ...
                                         size_t pattern_offset)
{
    ...
    if (!skb || !skb->data)
        return;
    pattern = (u32 *)(skb->data + pattern_offset);
    *pattern = 0;
...
void aml_amsdu_buf_list_deinit(struct aml_hw *aml_hw)
{
    ...
    /* coverity[NULL_FIELD], INIT_LIST_HEAD(&aml_hw->tx_amsdu_buf_free_list) is done in driver init */
    list_for_each_entry_safe(txamsdu, txamsdu_tmp, &aml_hw->tx_amsdu_buf_free_list, list) {
        kfree(txamsdu);
    }
    /* coverity[NULL_FIELD], INIT_LIST_HEAD(&aml_hw->tx_amsdu_buf_used_list) is done in driver init */
...
struct aml_txbuf *aml_get_from_free_txbuf(struct aml_hw *aml_hw)
{
    ...
    if (!list_empty(&aml_hw->tx_buf_free_list)) {
        /* coverity[NULL_FIELD], INIT_LIST_HEAD(&aml_hw->tx_buf_free_list) is done in driver init */
...
struct sk_buff *aml_get_skb_from_used_txbuf(struct aml_hw *aml_hw, u32_l hostid)
{
    ...
    spin_lock_bh(&aml_hw->tx_buf_lock);
    /* coverity[NULL_FIELD], INIT_LIST_HEAD(&aml_hw->tx_buf_used_list) is done in driver init */
...
{
    uint32_t cmd1[2] = { DYNAMIC_BUF_NOTIFY_FW_TX_STOP, 1};
    uint32_t cmd2[2] = { MAC_RST_NOTIFY_FW_TX_STOP, 1};

    if (aml_hw->dynabuf_stop_tx == DYNAMIC_BUF_HOST_TX_STOP)
        hi_sram_write(aml_hw, SDIO_USB_EXTEND_E2A_IRQ_STATUS, cmd1, sizeof(cmd1));

    if (aml_hw->mac_rst_stop_tx == MAC_RST_TX_STOP)
        hi_sram_write(aml_hw, SDIO_USB_EXTEND_E2A_IRQ_STATUS, cmd2, sizeof(cmd2));
}
...
int aml_tx_task(void *data)
{
    ...
    while (!aml_hw->aml_tx_task_quit) {
        ...
        if ((aml_hw->dynabuf_stop_tx == DYNAMIC_BUF_HOST_TX_STOP) || (aml_hw->mac_rst_stop_tx == MAC_RST_TX_STOP)) {
        ...
        /* coverity[NULL_FIELD], INIT_LIST_HEAD(&aml_hw->tx_desc_save) is done in driver init */
        list_for_each_entry_safe(sw_txhdr, next, &aml_hw->tx_desc_save, list) {
            ...
            if (aml_bus_type == USB_MODE) {
                ...
                if (sdio_txhdr && sdio_txhdr->cksum_flag & SDIO_TX_CKSUM_DATA_FLAG) {
                    ...
                    if (sw_txhdr->desc.api.host.flags & TXU_CNTRL_AMSDU) {
                        ...
                        if (frame_tot_len + SDIO_TXHEADER_LEN + SDIO_FRAME_TAIL_LEN >= 4096) {
                            ...
                            list_for_each_entry_safe(amsdu_txhdr, tmp, &sw_txhdr->amsdu.hdrs, list) {
                                if (!amsdu_txhdr->skb || !amsdu_txhdr->skb->data)
                                    break;
...
static u8 aml_msgind(void *pthis, void *arg)
{
    ...
    if (msg1 && msg1->pattern == IPC_MSGE2A_VALID_PATTERN) {
        if (msg1->id != 0)
...
int aml_traceind(struct aml_hw *aml_hw)
{
    ...
    if ((aml_bus_type == PCIE_MODE) || !g_trace_nl_info.enable
        || (atomic_read(&g_wifi_pm.bus_suspend_cnt) != 0) || atomic_read(&g_wifi_pm.drv_suspend_cnt)
...
void aml_ipc_tx_drain(struct aml_hw *aml_hw)
{
    ...
    while ((skb = ipc_host_tx_flush(aml_hw->ipc_env)) && skb->data) {
...
void aml_error_ind(struct aml_hw *aml_hw)
{
    ...
    struct dbg_debug_dump_tag *dump = NULL;
    if (!buf->addr)
        return;
