...
static void ipc_host_chan_switch_ind_handler(void *pthis)
{
    ...
    struct mm_channel_pre_switch_ind *pre_ind = (struct mm_channel_pre_switch_ind *)aml_hw->chan_switch_ind.param;
    ...
    if (ind_info.msg_id == MM_CHANNEL_PRE_SWITCH_IND) {
        aml_hw->chan_switch_ind.id = MM_CHANNEL_PRE_SWITCH_IND;
        pre_ind->chan_index = ind_info.chan_index;
    } else if (ind_info.msg_id == MM_CHANNEL_SWITCH_IND) {
        aml_hw->chan_switch_ind.id = MM_CHANNEL_SWITCH_IND;
        memcpy(&aml_hw->chan_switch_ind.param, &ind_info, sizeof(struct chan_switch_ind_info));
    } else {
        return;
    }
    aml_rx_sdio_ind_msg_handle(aml_hw, &aml_hw->chan_switch_ind);
...
static void ipc_usb_host_msgack_handler(struct ipc_host_env_tag *env)
{
    ...
    memset(&aml_hw->msg_ack, 0, sizeof(aml_hw->msg_ack));
    aml_hw->plat->hif_ops->hi_read_sram((unsigned char *)&aml_hw->msg_ack, (unsigned char *)&env->shared->msg_a2e_buf, sizeof(struct ipc_a2e_msg), USB_EP4);

    ASSERT_ERR(hostid);
    ASSERT_ERR(env->msga2e_cnt == (((struct lmac_msg *)(&aml_hw->msg_ack.msg))->src_id & 0xFF));
    if (!hostid) {
...
static void ipc_sdio_host_msgack_handler(struct ipc_host_env_tag *env)
{
    ...
    memset(&aml_hw->msg_ack, 0, sizeof(aml_hw->msg_ack));
    aml_hw->plat->hif_sdio_ops->hi_random_ram_read((unsigned char *)&aml_hw->msg_ack, (unsigned char *)&env->shared->msg_a2e_buf, sizeof(struct ipc_a2e_msg));
...
uint8_t aml_get_dbg_info(struct aml_hw *aml_hw)
{
    ...
    if ((atomic_read(&g_wifi_pm.bus_suspend_cnt) != 0) || atomic_read(&g_wifi_pm.is_shut_down)
        || bus_state_detect.bus_err) {
...
#endif
    {
        aml_hw->plat->hif_ops->hi_read_sram((unsigned char *)dbg_info,
            (unsigned char *)ASSERT_INFO_HOST_ADDR, DBG_INFO_LEN, USB_EP4);
    }
...
uint8_t aml_get_dbg_trace_data(struct aml_hw *aml_hw)
{
    ...
    if (trace_log_file_info.assert_ptr == NULL)
        return 0;
...
void aml_sdio_usb_extend_irq_handle(struct aml_hw *aml_hw)
{
    ...
    switch (status)
    {
        case MAC_RST_TX_STOP:
            AML_INFO("MAC_RST_TX_STOP!\n");
            aml_hw->mac_rst_stop_tx = MAC_RST_TX_STOP;
            aml_hw->send_tx_stop_to_fw = 1;
            up(&aml_hw->aml_tx_sem);
            break;

        case MAC_RST_TX_START:
            AML_INFO("MAC_RST_TX_START!\n");
            aml_hw->mac_rst_stop_tx = 0;
            up(&aml_hw->aml_tx_sem);

            if (aml_hw->mac_rst_defer_e2a) {
                aml_hw->mac_rst_defer_e2a = 0;
                if (aml_hw->ipc_env != NULL)
...
int ipc_host_msg_push(struct ipc_host_env_tag *env, void *msg_buf, uint16_t len)
{
    ...
    while (aml_hw->mac_rst_stop_tx) {
        if (cnt == 20)
            break;

        AML_INFO("msg %d wait %d mac/phy reset", ((struct aml_cmd *)msg_buf)->id, cnt);
        msleep(10);
        cnt++;
    }
