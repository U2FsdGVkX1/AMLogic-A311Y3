...
int aml_send_start(struct aml_hw *aml_hw)
{
    ...
    start_req_param->rx_hostbuf_size = aml_hw->ipc_env ? (u16_l)aml_hw->ipc_env->rxbuf_sz : 0;
...
int aml_send_open_close_if(struct aml_hw *aml_hw, int open_close)
{
    int ret = 0;
    unsigned int *vif_open_close = NULL;

    vif_open_close = aml_priv_msg_zalloc(MM_SUB_OPEN_CLOSE, sizeof(unsigned int));
    if (!vif_open_close)
        return -ENOMEM;
    *vif_open_close = open_close;
    ret = aml_priv_send_msg(aml_hw, vif_open_close, 0, MM_MSG_BYPASS_ID, NULL);
    AML_INFO("send MM_SUB_OPEN_CLOSE ret %d vif_open_close %d\n",ret, open_close);
                         ...
                         const u8 *mac, u8 inst_nbr, struct me_sta_add_cfm *cfm)
{
    ...
    if (P_LINK_STA_PARAMS(params, ht_capa)) {
        ...
        ht_mcs = (u8 *)&(P_LINK_STA_PARAMS(params, ht_capa->mcs));
                            ...
                            struct apm_start_cfm *cfm)
{
    ...
    list_for_each_entry(aml_vif, &aml_hw->vifs, list) {
        ...
        if (AML_VIF_TYPE(aml_vif) == NL80211_IFTYPE_STATION) {
            while (aml_connect_flags_chk(aml_vif, AML_CONNECTING)) {
                msleep(20);
                if (cnt++ > 100) {
                    AML_INFO("sta is connecting ap, sleep %d connect_flags: %x\n", 20 * cnt, aml_vif->sta.connect_flags);
                    break;
                }
            }
        }
...
#endif
    {
        ...
        band = settings->chandef.chan ? settings->chandef.chan->band : NL80211_BAND_2GHZ;
    }
...
int aml_send_extra_ssid_param(struct aml_hw *aml_hw, struct cfg80211_scan_request *param)
{
    ...
    for (i = 0; i < ssid_param->ssid_cnt; i++) {
        ...
        ssid_index = i + SCAN_SSID_MAX;
        ssid_len = param->ssids[ssid_index].ssid_len;
        if (ssid_len < 0 || ssid_len > sizeof(ssid_param->ssid[i].array)) {
            AML_ERR("invalid SSID length: %d", ssid_len);
            ssid_len = min_t(int, ssid_len, sizeof(ssid_param->ssid[i].array));
        }

        for (j = 0; j < ssid_len; j++)
            ssid_param->ssid[i].array[j] = param->ssids[ssid_index].ssid[j];

        ssid_param->ssid[i].length = ssid_len;
...
#define AML_CHAN_AWARE_CNT (10)
extern int aml_find_channel_index(struct scan_5gchan_info *chan_set, u16 freq);
                        ...
                        struct cfg80211_scan_request *param)
{
    ...
    static uint8_t dfs_chan_aware = 0;
    ...
    if ((req->ssid_cnt == 0) || (aml_hw->active_scan_enable == 0))
        chan_flags |= CHAN_NO_IR;

    if (dfs_chan_aware > AML_CHAN_AWARE_CNT) {
        for (j = 0; j < MAC_DOMAINCHANNEL_5G_MAX; j++) {
            aml_hw->scan_5g_channels[j].flags = AML_CHAN_NO_AWARE;
        }
        dfs_chan_aware = 0;
    }
    for (i = 0; i < req->ssid_cnt; i++) {
    ...
    for (i = 0; i < req->chan_cnt; i++) {
        ...
        req->chan[i].freq = chan->center_freq;
        req->chan[i].flags = chan_flags | get_chan_flags(chan->flags);
        if ((chan_flags == 0) && (req->chan[i].flags & (CHAN_NO_IR | CHAN_RADAR))) {
            int chan_idx = aml_find_channel_index(aml_hw->scan_5g_channels, req->chan[i].freq);
                 if ((chan_idx != -1) && ((aml_hw->scan_5g_channels[chan_idx].flags == AML_CHAN_AWARE))) {
                    req->chan[i].flags = 0;
                 }
        }
#ifdef CONFIG_SCAN_RANDOM_MAC_ADDR
#if (CFG80211_VERSION_CODE >= KERNEL_VERSION(3, 19, 0))
        if (param->flags & NL80211_SCAN_FLAG_RANDOM_ADDR) {
            req->chan[i].flags |= CHAN_RANDOM_ADDR_SCAN;
        }
#endif
#endif
...
#endif /* CONFIG_AML_SOFTMAC*/
{
    ...
    bfmer_en_req->host_bfr_addr = aml_sta->bfm_report ? aml_sta->bfm_report->dma_addr : 0;
...
int aml_set_suspend_tx_flush(struct aml_hw *aml_hw, int tx_flush_enable)
{
    ...
    AML_INFO("suspend notify fw tx flush host_cfm_idx:%d, fw_cfm_idx:%d\n",
        aml_hw->ipc_env ? aml_hw->ipc_env->txcfm_idx : 123456, AML_REG_READ(aml_hw->plat, 0, SRAM_SYNC_FW_CFM_IDX));
                                  ...
                                  struct scanu_sched_scan_start_req *sched_start_req)
{
    ...
    if (request->min_rssi_thold >= -128)
        sched_start_req->min_rssi_thold = request->min_rssi_thold;
    else
        sched_start_req->min_rssi_thold = -128;
    ...
    for (i = 0; i < sched_start_req->n_scan_plans; i++) {
     ...
     for (i = 0; i < sched_start_req->match_count; i++) {
        ...
        if (request->min_rssi_thold >= -128)
            ...
            sched_start_req->match_sets[i].rssiThreshold = -128;
        AML_INFO("match_sets[%d] uplayer req %d, drv set %d\n", i, request->match_sets[i].rssi_thold, sched_start_req->match_sets[i].rssiThreshold);
#endif
...
{
    struct sched_scan_stop_cfm cfm;
    struct aml_hw *aml_hw = aml_vif->aml_hw;
    ...
    /* coverity[leaked_storage] - sched_start_req will be freed later */
    return aml_priv_send_msg(aml_hw, sched_scan_stop_req, 1, PRIV_SCANU_SCHED_STOP_CFM, &cfm);
}
...
{
    int i;
    struct mm_mdns_set_wake_ports *req;

    if (!ports)
        return -ENOMEM;

    req = aml_priv_msg_zalloc(MDNS_SET_WAKE_PORTS, sizeof(struct mm_mdns_set_wake_ports));
    if (!req)
        return -ENOMEM;
    ...
    AML_INFO("set mdns wake port ok:\n");
    /* coverity[leaked_storage] - req will be freed later */
    return aml_priv_send_msg(aml_hw, req, 0, 0, NULL);
}
...
int aml_set_coex_lead_time(struct aml_hw *aml_hw, u8 lead_time)
{
    struct coex_lead_time *coex_lead_time_param = NULL;

    if (lead_time < 1 || lead_time > 7)
    {
        AML_INFO("set coex_lead_time err: %d", lead_time);
        lead_time = 4;
    }

    coex_lead_time_param = aml_priv_msg_zalloc(MM_SUB_COEX_LEAD_TIME, sizeof(struct coex_cmd_req));

    if (!coex_lead_time_param)
        return -ENOMEM;

    coex_lead_time_param->lead_time = lead_time;

    AML_INFO("set coex_lead_time: %d", lead_time);
    /* coverity[leaked_storage] - coex_lead_time_param will be freed later */
    return aml_priv_send_msg(aml_hw, coex_lead_time_param, 0, 0, NULL);
}

int aml_set_coex_rssi_step(struct aml_hw *aml_hw, struct Coex_rssi_param coex_rssi_para)
{
    struct Coex_rssi_step *rssi_step_param = NULL;

    rssi_step_param = aml_priv_msg_zalloc(MM_SUB_COEX_RSSI_STEP, sizeof(struct Coex_rssi_step));

    if (!rssi_step_param)
        return -ENOMEM;

    if (coex_rssi_para.coex_rssi_step1 > 40 || coex_rssi_para.coex_rssi_step1 < -40)
    {
        AML_INFO("set coex_rssi_step1 err: %d",coex_rssi_para.coex_rssi_step1);
        coex_rssi_para.coex_rssi_step1 = 0;
    }

    if (coex_rssi_para.coex_rssi_step2 > 60 || coex_rssi_para.coex_rssi_step2 < -60)
    {
        AML_INFO("set coex_rssi_step2 err: %d",coex_rssi_para.coex_rssi_step2);
        coex_rssi_para.coex_rssi_step2 = 0;
    }

    rssi_step_param->rssi_step1 = coex_rssi_para.coex_rssi_step1;
    rssi_step_param->rssi_step2 = coex_rssi_para.coex_rssi_step2;

    AML_INFO("set coex_rssi_step: %d %d",rssi_step_param->rssi_step1, rssi_step_param->rssi_step2);
    /* coverity[leaked_storage] - rssi_step_param will be freed later */
    return aml_priv_send_msg(aml_hw, rssi_step_param, 0, 0, NULL);
}

int aml_set_coex_sink_mode(struct aml_hw *aml_hw, u8 sink_mode)
{
    struct coex_sink_mode *coex_sink_param = NULL;

    coex_sink_param = aml_priv_msg_zalloc(MM_SUB_COEX_SINK_MODE, sizeof(struct coex_sink_mode));

    if (!coex_sink_param)
        return -ENOMEM;

    coex_sink_param->coex_sink_mode = sink_mode;

    AML_INFO("set coex_sink_mode: %d", sink_mode);
    /* coverity[leaked_storage] - coex_sink_param will be freed later */
    return aml_priv_send_msg(aml_hw, coex_sink_param, 0, 0, NULL);
}
...
int aml_send_sync_trace(struct aml_hw *aml_hw)
{
    ...
    if (atomic_read(&g_wifi_pm.drv_suspend_cnt) == 1 || atomic_read(&g_wifi_pm.bus_suspend_cnt) != 0) {
        return -1;
    }
...
int aml_send_set_suspend_irq_inform(struct aml_hw *aml_hw)
{
    if (aml_bus_type == SDIO_MODE)
    {
        AML_INFO("suspend_test_no_ack: \n");
        aml_irq_sdio_thread(aml_hw->irq, aml_hw);
    }
    return 0;
}

void aml_suspend_no_acktimer_cb(struct timer_list *t)
{
    if (aml_bus_type == SDIO_MODE)
    {
        struct aml_hw *aml_hw = from_timer(aml_hw, t, suspend_noack_timer);
        aml_wq_do(aml_send_set_suspend_irq_inform, aml_hw);
    }
}

void aml_suspend_no_ack_init(struct aml_hw *aml_hw)
{
    if (aml_bus_type == SDIO_MODE)
    {
        timer_setup(&aml_hw->suspend_noack_timer, aml_suspend_no_acktimer_cb, 0);
    }
}
#endif
#endif
...
int aml_send_scan_param_req(struct aml_hw *aml_hw, u16 non_dfs_chan_scan_ms, u16 dfs_chan_scan_ms, u16 probe_num)
{
    struct mm_set_scan_param_req *req = aml_priv_msg_zalloc(MM_SUB_SET_SCAN_PARAM_REQ, sizeof(struct mm_set_scan_param_req));

    if (!req)
        return -ENOMEM;

    if ((aml_partner_cust == ROKU_DONGLE_VER) || (aml_partner_cust == ROKU_TV_VER)) {
        req->non_dfs_chan_scan_ms = non_dfs_chan_scan_ms;
        req->dfs_chan_scan_ms = dfs_chan_scan_ms;
        req->probe_num = probe_num;
    } else {
        req->dfs_chan_scan_ms = dfs_chan_scan_ms;
        req->p2p_scan_ch1_ms = aml_hw->custom_scan_duration.p2p_scan_ch1_ms;
        req->p2p_scan_ch3_ms = aml_hw->custom_scan_duration.p2p_scan_ch3_ms;
        req->specified_scan_ch_ms = aml_hw->custom_scan_duration.specified_scan_ch_ms;
        req->no_sched_scan_ch_ms = aml_hw->custom_scan_duration.no_sched_scan_ch_ms;
        req->other_scan_ch_ms = aml_hw->custom_scan_duration.other_scan_ch_ms;

        req->wfd_conn_scan_delay = aml_hw->custom_scan_duration.wfd_conn_scan_delay;
        req->p2p_conn_scan_delay = aml_hw->custom_scan_duration.p2p_conn_scan_delay;
        req->sta_conn_scan_delay = aml_hw->custom_scan_duration.sta_conn_scan_delay;
    }
    /* coverity[leaked_storage] - req will be freed later */
    return aml_priv_send_msg(aml_hw, req, 0, MM_MSG_BYPASS_ID, NULL);
}
...
int aml_set_linkloss_threshold(struct aml_hw *aml_hw, int threshold)
{
    ...
    if ((threshold < 3000 * 1024) && (threshold != 0)) {  // 3000 * TU ms, 0 is default
        AML_ERR("error param threshold:%d", threshold);
        return EINVAL;
    }
    ...
    AML_INFO("set threshold:%d", threshold);
...
int aml_set_2g4_bandwidth(struct aml_hw * aml_hw, u8 enable_2g4_20m)
{
    struct mm_2g4_bandwidth *req;

    if ((enable_2g4_20m != 0) && (enable_2g4_20m != 1)) {
        AML_ERR("error param enable_2g4_20m:%d", enable_2g4_20m);
        return EINVAL;
    }

    req = aml_priv_msg_zalloc(MM_SUB_SET_2G4_BANDWIDTH, sizeof(struct mm_2g4_bandwidth));
    if (!req)
        return -ENOMEM;
    ...
    if (aml_bus_type == USB_MODE) {
        ...
        if (align_size != 0)
        {
           aml_plat->hif_ops->hi_write_sram((unsigned char *)program,
               (unsigned char *)aml_hw->apf_params.apf_cap.apf_mem_addr, align_size, USB_EP4);
        }

        if (program_len % 512 != 0)
        {
           aml_plat->hif_ops->hi_write_sram((unsigned char *)program + align_size,
               (unsigned char *)(aml_hw->apf_params.apf_cap.apf_mem_addr + aml_hw->apf_params.apf_cap.max_len), program_len % 512, USB_EP4);
        }
    }
#ifdef SDIO_MODE_ON
    else if (aml_bus_type == SDIO_MODE) {
...
#ifdef    CONFIG_AML_WOW_BROADCAST_MAGIC_PACKET_EN
    apf_set_mode_req->broad_magic_wake_bypass = true;
#else
    apf_set_mode_req->broad_magic_wake_bypass = false;
#endif
