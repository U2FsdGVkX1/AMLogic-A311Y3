...
enum priv_e2a_tag {
    ...
    PRIV_MCC_CHECK_NOTIFY,
    PRIV_DPD_INFORM_DRIVER_IND,
    PRIV_SCANU_SCHED_STOP_CFM,
    PRIV_SUB_E2A_MAX,
};
...
enum mm_sub_a2e_tag {
    ...
    MM_SUB_OPEN_CLOSE,
    MM_SUB_SET_LINKLOSS_THRESHOLD,
    MM_SUB_SET_2G4_BANDWIDTH,
    MM_SUB_SET_EXTRA_SSID_PARAM,
    MM_SUB_SET_SCAN_PARAM_REQ,
    MDNS_SET_WAKE_PORTS,
    MM_SUB_COEX_LEAD_TIME,
    MM_SUB_COEX_RSSI_STEP,
    MM_SUB_COEX_SINK_MODE,
    /// the MAX
    MM_SUB_A2E_MAX,
...
struct coex_lead_time
{
    u8_l lead_time;
};

struct coex_sink_mode
{
    u8_l coex_sink_mode;
};

struct Coex_rssi_step
{
    int8_t rssi_step1;
    int8_t rssi_step2;
};
...
struct csi_set_runtime_req
{
    ...
    u32_l time_total;
    u8_l dest_ip[4];/* dest ip */
    u8_l source_ip[4];/* source ip */
    u8_l addr1[ETH_ALEN];
    u8_l addr2[ETH_ALEN];
    u8_l addr3[ETH_ALEN];
    u8_l sta_or_sap;//0:sta,2:sap
    u8_l res;
};
...
struct mm_set_scan_param_req
{
    u16_l non_dfs_chan_scan_ms;
    u16_l dfs_chan_scan_ms;
    u16_l p2p_scan_ch1_ms;
    u16_l p2p_scan_ch3_ms;
    u16_l specified_scan_ch_ms;
    u16_l no_sched_scan_ch_ms;
    u16_l other_scan_ch_ms;
    uint16_t wfd_conn_scan_delay;
    uint16_t p2p_conn_scan_delay;
    uint16_t sta_conn_scan_delay;
    u16_l probe_num;
};
...
struct apf_set_mode_req
{
    ...
    bool broad_magic_wake_bypass; //android 16 need bypass broadcast magic to wake host
    u8_l resvd[2];
};
