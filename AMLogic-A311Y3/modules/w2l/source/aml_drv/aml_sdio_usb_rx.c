...
#define AML_RX_MON_CHECK_INTERVAL   (AML_RX_MON_IDLE_THRESHOLD / 8 / USEC_PER_MSEC)
...
{
    return (struct hw_rxhdr *)&rxdesc->dma_hdrdesc.hd.frmlen;
}
...
static void aml_rx_idle_enter(struct aml_rx *rx)
{
    ...
    if (show || !test_bit(AML_RX_STATE_IDLE, &rx->state)) {
        ...
        AML_ERR("RX IDLE: [%x, %x] [%x, %x]\n",
                ...
                AML_REG_READ(plat, 0, CMD_DOWN_FIFO_FDH_ADDR + 4));
    ...
    while (len > 0 && rxdesc) {
        ...
        if (tot_len < 0) {
            ...
            if (RX_PD_LEN && len < RX_DESC_SIZE + RX_PD_LEN) {
                ...
                if (len > pkt_len + 16 + sizeof(u32)) {    /* pkt_len + icv(GCMP_TAIL_LEN=16) + fcs */
...
static inline int aml_sdio_usb_rx_desc_has_pending(struct aml_rx *rx)
{
    ...
    if (!rx->buf)
        return 0;
    pos = *aml_rx_desc_next_ptr(rx->buf + rx->tail) & ~AML_RX_WRAP_FLAG;
