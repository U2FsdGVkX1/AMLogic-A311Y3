...
static void aml_rx_assoc_req(struct aml_hw *aml_hw, struct sk_buff *skb)
{
    ...
    if ((aml_partner_cust == ROKU_DONGLE_VER) || (aml_partner_cust == ROKU_TV_VER)) {
        if (cfg80211_find_vendor_ie(ROKU_ASOC_VENDOR_IE, ROKU_ASOC_VENDOR_TYPE, ies, skb->data + skb->len - ies)) {
            AML_WARN("roku remote assoc req, addr:%pM", mgmt->sa);
            aml_hw->rx_assoc_info.legacy_device = true;
        }
        else {
            aml_hw->rx_assoc_info.legacy_device = false;
        }
    }
}
    ...
    if (ieee80211_is_beacon(mgmt->frame_control)) {
        ...
        if (ieee80211_is_assoc_req(mgmt->frame_control) || ieee80211_is_reassoc_req(mgmt->frame_control)) {
            ...
            if (ieee80211_is_auth(mgmt->frame_control)) {
                ...
                struct aml_sta *sta = aml_get_sta(aml_hw, mgmt->sa);
                if (sta && !sta->is_disconnecting) {
                    AML_INFO("rx deauth, set disconnecting state sta_idx:%d, addr:%pM", sta->sta_idx, sta->mac_addr);
                    sta->is_disconnecting = 1;
                }
            }
        }
...
u8 aml_unsup_rx_vec_ind(void *pthis, void *arg) {
    ...
    skb = ipc_buf->addr;
    if (!skb || !skb->data || (((struct rx_vector_desc *)(skb->data))->pattern == 0)) {
