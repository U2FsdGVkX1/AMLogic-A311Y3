...
static int aml_sdio_usb_irq_task(struct aml_hw *aml_hw)
{
    ...
    while ((status = aml_hw->plat->ack_irq(aml_hw))) {
        ...
        if ((aml_hw->mac_rst_stop_tx) && (status & SDIO_IRQ_E2A_MSG)) {
            aml_hw->mac_rst_defer_e2a = 1;
            status = status & ~SDIO_IRQ_E2A_MSG;
        }
...
irqreturn_t aml_irq_sdio_thread(int irq, void *dev_id)
{
    struct aml_hw *aml_hw = (struct aml_hw *)dev_id;
    AML_INFO("irq=%d ,bus_suspend_cnt = %d, shut_down= %d, irq_done = %d\n",
            aml_hw->irq, atomic_read(&g_wifi_pm.bus_suspend_cnt), atomic_read(&g_wifi_pm.is_shut_down), aml_hw->irq_done);
...
int aml_usb_irq_urb_submit(struct aml_hw *aml_hw)
{
    ...
    if (urb->status != -EINPROGRESS) {
        ...
        req = (struct usb_ctrlrequest *)aml_hw->usb->urb.setup_packet;
        req->wIndex++;
...
void aml_pcie_task(unsigned long data)
{
    ...
    while ((aml_hw->ipc_env) && (status = ipc_host_get_status(aml_hw->ipc_env))) {
