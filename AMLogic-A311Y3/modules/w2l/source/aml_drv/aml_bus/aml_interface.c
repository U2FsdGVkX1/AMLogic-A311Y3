...
unsigned int aml_usb_hotplug = 0;
...
void init_custom_ver(void)
{
    ...
    } else if (strncmp(partner_cust, "fenda", 5) == 0)
    {
        aml_partner_cust = FENDA_VER;
    }
    if ((aml_partner_cust == ROKU_DONGLE_VER)
        || (aml_partner_cust == ROKU_TV_VER)
        || (aml_partner_cust == TCL_TV_VER))
        aml_usb_hotplug = 1;
}
