...
void aml_txq_flush(struct aml_hw *aml_hw, struct aml_txq *txq)
{
    ...
    if (pushed)
        AML_INFO("TXQ[%d]: %d skb still pushed to the FW",
                 txq->idx, pushed);
}
...
static void aml_txq_drop_dump(struct aml_vif *vif, const char *txq_name)
{
    ...
    if (((aml_partner_cust == ROKU_DONGLE_VER) || (aml_partner_cust == ROKU_TV_VER))
        ...
        if (!list_empty(&vif->ap.sta_list)) {
            ...
            /* coverity[NULL_FIELD], INIT_LIST_HEAD(&vif->ap.sta_list) is done in driver init */
...
static void aml_txq_cleanup_timer_cb(struct timer_list *t)
{
    ...
    /* coverity[NULL_FIELD], INIT_LIST_HEAD(&aml_hw->vifs) is done in driver init */
                       ...
                       struct sk_buff *skb_prev)
{
    ...
    if ((txq->ndev_idx != NDEV_NO_TXQ) &&
        (skb_queue_len(&txq->sk_list) > ((aml_hw->mod_params->wfa_en && aml_hw->twt_en) ? AML_NDEV_FLOW_CTRL_STOP_FOR_WFA_TWT : AML_NDEV_FLOW_CTRL_STOP))) {
        txq->status |= AML_TXQ_NDEV_FLOW_CTRL;
...
void aml_hwq_process(struct aml_hw *aml_hw, struct aml_hwq *hwq)
{
    ...
    /* coverity[NULL_FIELD], INIT_LIST_HEAD(&hwq->list) is done in driver init */
    list_for_each_entry_safe(txq, next, &hwq->list, sched_list) {
        ...
        while ((skb = __skb_dequeue(&sk_list_push)) != NULL && skb->data) {
        ...
        if (unlikely(txq->status & AML_TXQ_NDEV_FLOW_CTRL) &&
            skb_queue_len(&txq->sk_list) < ((aml_hw->mod_params->wfa_en && aml_hw->twt_en) ? AML_NDEV_FLOW_CTRL_RESTART_FOR_WFA_TWT : AML_NDEV_FLOW_CTRL_RESTART)) {
            txq->status &= ~AML_TXQ_NDEV_FLOW_CTRL;
            netif_wake_subqueue(txq->ndev, txq->ndev_idx);
            trace_txq_flowctrl_restart(txq);
        }
