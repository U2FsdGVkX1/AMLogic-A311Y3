    ...
    struct wireless_dev *wdev, const void *data, int len)
{
        ...
        switch (type) {
            case WIFI_MDNS_OFFLOAD_ATTRIBUTE_WAKE_PORTS_NUM:
                ...
                if (num > MDNS_WAKE_PORT_MAX) {
                    AML_M_INFO(MDNS, "mdnsOffload: warning for wake port num, is :%u\n", num);
            ...
            case WIFI_MDNS_OFFLOAD_ATTRIBUTE_WAKE_PORTS:
                ...
                break;
            default:
                AML_M_INFO(MDNS,"mdnsOffload: unknown type:%d\n", type);
                break;
        }
    }
    if (!p_num || !port) {
    ...
    AML_M_INFO(MDNS, "mdnsOffload: setWakePorts: num:%u\n", num);
    ...
    if (mdns_offload_ops.setWakePorts) {
        ...
        AML_M_INFO(MDNS, "mdnsOffload: setWakePorts: unsupported!\n");
        err = -EPERM;
        goto exit;
    }
exit:
    if (port)
        kfree(port);
    if (err)
        AML_M_ERR(MDNS, "mdnsOffload: setWakePorts: failed!err:%d\n", err);
    return err;
}
    ...
    struct wireless_dev *wdev, const void *data, int len)
{
    ...
    AML_M_INFO(MDNS, "mdnsOffload: addToPassthroughList: ifname:%s, length:%d qname:%s\n", ifname, strlen(qname), qname);
    ...
    struct wireless_dev *wdev, const void *data, int len)
{
    ...
    AML_M_WARN(MDNS, "mdnsOffload: removeFromPassthroughList: ifname:%s,qname:%s\n", ifname, qname);
    if (!p_ifname || !p_qname) {
        err = -EINVAL;
        goto exit;
    }
    ...
    struct wireless_dev *wdev, const void *data, int len)
{
    ...
    AML_M_INFO(MDNS, "mdnsOffload: setPassthroughBehavior: ifname:%s, behavior:%d\n", ifname, behavior);
