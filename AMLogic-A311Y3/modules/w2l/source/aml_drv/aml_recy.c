...
{
    if (!aml_recy) {
        AML_DBG("Current task: %s, flags(0x%0x)\n", current->comm, flags);
        return false;
    }
...
Try_again:
    ...
    if (bus_state_detect.bus_err == 1 || atomic_read(&g_wifi_pm.is_shut_down)
        || (atomic_read(&g_wifi_pm.bus_suspend_cnt) != 0)) {
...
static int aml_recy_vif_restart(struct aml_hw *aml_hw)
{
    ...
    for (i = 0; i < NX_VIRT_DEV_MAX; i++) {
        ...
        if (AML_VIF_TYPE(aml_vif) == NL80211_IFTYPE_AP && aml_recy_flags_chk(AML_RECY_AP_INFO_SAVED)) {
