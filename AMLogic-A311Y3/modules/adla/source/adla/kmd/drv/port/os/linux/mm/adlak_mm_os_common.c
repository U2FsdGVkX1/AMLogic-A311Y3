...
static int adlak_dma_map_of_discontiguous(struct adlak_mem *mm, struct adlak_mem_handle *mm_info,
    ...
    mm_info->sgt = adlak_os_vmalloc(sizeof(struct sg_table), ADLAK_GFP_KERNEL);
    if (!mm_info->sgt) {
...
void adlak_os_free_discontiguous(struct adlak_mem *mm, struct adlak_mem_handle *mm_info) {
    ...
    if (mm_info->pages) {
        ...
        adlak_os_vfree(mm_info->phys_addrs);
        adlak_os_vfree(mm_info->pages);
        mm_info->pages = NULL;
    }
}
...
int adlak_os_alloc_discontiguous(struct adlak_mem *mm, struct adlak_mem_handle *mm_info) {
    ...
    pages = (struct page **)adlak_os_vzalloc(sizeof(struct page *) * mm_info->nr_pages,
                                             ADLAK_GFP_KERNEL);
    if (!pages) {
    ...
    phys_addrs =
        (phys_addr_t *)adlak_os_vzalloc(sizeof(phys_addr_t) * mm_info->nr_pages, ADLAK_GFP_KERNEL);
...
int adlak_os_attach_ext_mem(struct adlak_mem *mm, struct adlak_mem_handle *mm_info,
    ...
    /* ext mem must be contiguous + uncacheable */
...
void adlak_os_flush_cache(struct adlak_mem *mm, struct adlak_mem_handle *mm_info,
    ...
    AML_LOG_DEBUG(
        "%s mem_type %lX\tiova_addr %lX\tcpu_addr %lX\t"
        "cpu_addr_user %lX \nis_partial %d\toffset %lX\tsize %lX\n",
        __func__, (uintptr_t)mm_info->mem_type, (uintptr_t)mm_info->iova_addr,
        (uintptr_t)mm_info->cpu_addr, (uintptr_t)mm_info->cpu_addr_user,
        (int32_t)sync_cache_ext_info->is_partial, (uintptr_t)sync_cache_ext_info->offset,
...
void adlak_os_invalid_cache(struct adlak_mem *mm, struct adlak_mem_handle *mm_info,
    ...
    AML_LOG_DEBUG(
        ...
        __func__, (uintptr_t)mm_info->mem_type, (uintptr_t)mm_info->iova_addr,
        (uintptr_t)mm_info->cpu_addr, (uintptr_t)mm_info->cpu_addr_user,
        sync_cache_ext_info->is_partial, (uintptr_t)sync_cache_ext_info->offset,
        (uintptr_t)sync_cache_ext_info->size);
    if (ADLAK_ENUM_MEMTYPE_INNER_CONTIGUOUS & mm_info->mem_type) {
        ASSERT(mm_info->dma_addr);
        if (0 == sync_cache_ext_info->is_partial) {
