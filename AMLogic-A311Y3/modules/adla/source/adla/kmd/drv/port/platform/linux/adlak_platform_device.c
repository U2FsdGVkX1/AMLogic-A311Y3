...
#ifdef CONFIG_ADLAK_ENABLE_DEBUGFS
    adlak_destroy_debugfs(padlak);
#endif
...
#ifdef CONFIG_ADLAK_ENABLE_DEBUGFS
    ret = adlak_create_debugfs(padlak);
    if (ret) {
...
#ifdef CONFIG_ADLAK_TEE
    /*init secure mem */
    ret = adlak_tee_mem_init(padlak);
    if (ret) {
        AML_LOG_ERR(" Adla Tee mem init fail!");
        goto err_create_dev;
    }
#endif
