...
#define ADLAK_DEBUG_MALLOC 0
/************************** Variable Definitions *****************************/
#if ADLAK_DEBUG_MALLOC
...
void *adlak_os_malloc(size_t size, uint32_t flag) {
    ...
    return addr;
#else
    if (size > (16 * ADLAK_PAGE_SIZE)) {
        AML_LOG_WARN("alloc size 0x%lX may fail. It is recommended to use vmalloc \n",
                     (uintptr_t)size);
    }
...
#else
    if (size > (16 * ADLAK_PAGE_SIZE)) {
        AML_LOG_WARN("alloc size 0x%lX may fail. It is recommended to use vzalloc \n",
                     (uintptr_t)size);
    }
...
int64_t adlak_os_get_mem_available(void) {
    unsigned long avail_pages;
    int64_t       avail_bytes;
    avail_pages = si_mem_available();
    avail_bytes = avail_pages * ADLAK_PAGE_SIZE;
    return (int64_t)avail_bytes;
}