...
static void adlak_smmu_free_inner(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
                                  struct adlak_mem_handle *mm_info, uint8_t has_iova);

static struct adlak_mem_handle *adlak_smmu_alloc_inner(
    void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr, struct adlak_buf_req *pbuf_req,
    uint8_t has_iova);
static void *adlak_smmu_vmap(struct adlak_mem_handle *mm_info);

static void inline adlak_smmu_claim_dev(struct adlak_mem_smmu *ptr_mm_smmu) {
    AML_LOG_DEBUG("%s[+]", __func__);
    adlak_os_mutex_lock(&ptr_mm_smmu->mutex);
    AML_LOG_DEBUG("%s[-]", __func__);
}
static void inline adlak_smmu_release_dev(struct adlak_mem_smmu *ptr_mm_smmu) {
    AML_LOG_DEBUG("%s[+]", __func__);
    adlak_os_mutex_unlock(&ptr_mm_smmu->mutex);
    AML_LOG_DEBUG("%s[-]", __func__);
}

static int adlak_smmu_tlb_alloc(struct adlak_mem_smmu *   ptr_mm_smmu,
                                struct adlak_smmu_object *ptr_smmu) {
    int                         ret = 0;
    struct adlak_mem_handle *   mm_info;
    int                         idx1, idx2;
    ...
    tlb->tlb_info =
        adlak_os_vmalloc(tlb_info_count * sizeof(struct adlak_smmu_tlb_info), ADLAK_GFP_KERNEL);
    ...
    mm_info       = adlak_smmu_alloc_inner(ptr_mm_smmu, NULL, &buf_req, 0);
...
static int adlak_smmu_tlb_fill_init(struct adlak_mem_smmu *   ptr_mm_smmu,
                                    struct adlak_smmu_object *ptr_smmu) {
    int                         idx1, idx2, idx3;
    uintptr_t                   tlb_logic;
    ...
    for (idx1 = 0; idx1 < tlb->iova_size_GB; idx1++) {
        ...
        for (idx2 = 0; idx2 < SMMU_TLB2_ENTRY_COUNT_2M; idx2++) {
            ...
            for (idx3 = 0; idx3 < SMMU_TLB2_ENTRY_COUNT_2M;) {
                iova = (idx1 * 0x40000000) + (idx2 * 0x200000) + (idx3 * 4096);
#ifdef CONFIG_64BIT
                adlak_os_printf(
                    "iova 0x%lX : offset:0x%08X\t0x%lX 0x%lX 0x%lX 0x%lX \n", (uintptr_t)iova,
...
#else
                adlak_os_printf(
                    "iova 0x%lX : offset:0x%08X\t0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X "
                    "0x%08X\n",
                    (uintptr_t)iova, (uint32_t)(idx3 * sizeof(uint64_t)),
...
static void adlak_smmu_tlb_dump(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr) {
#ifdef CONFIG_ADLAK_DEBUG_SMMU_TLB_DUMP
    struct adlak_mem_smmu *   ptr_mm_smmu  = obj;
    struct adlak_smmu_object *smmu_public  = NULL;
    struct adlak_smmu_object *ptr_smmu_pri = NULL;

    adlak_os_printf("%s", __func__);
    if (ctx_smmu_attr) {
        adlak_smmu_claim_dev(ptr_mm_smmu);
        ...
        adlak_smmu_release_dev(ptr_mm_smmu);
    }
#endif
}
...
static int adlak_smmu_tlb_deinit(struct adlak_mem_smmu *   ptr_mm_smmu,
                                 struct adlak_smmu_object *ptr_smmu) {
    int                    idx1, idx2;
    struct adlak_smmu_tlb *tlb = NULL;
    AML_LOG_DEBUG("%s", __func__);
    ...
    if (tlb->tlb_info) {
        if (tlb->ptr_tlb_l1->mm_info) {
            adlak_smmu_free_inner(ptr_mm_smmu, NULL, tlb->ptr_tlb_l1->mm_info, 0);
        }

        for (idx1 = 0; idx1 < tlb->iova_size_GB; idx1++) {
            if (tlb->ptr_tlb_l2[idx1].mm_info) {
                adlak_smmu_free_inner(ptr_mm_smmu, NULL, tlb->ptr_tlb_l2[idx1].mm_info, 0);
            }
        }
        for (idx1 = 0; idx1 < tlb->iova_size_GB; idx1++) {
            for (idx2 = 0; idx2 < SMMU_TLB2_ENTRY_COUNT_2M; idx2++) {
                if (tlb->ptr_tlb_l3[idx1 * SMMU_TLB2_ENTRY_COUNT_2M + idx2].mm_info) {
                    adlak_smmu_free_inner(
                        ptr_mm_smmu, NULL,
                        tlb->ptr_tlb_l3[(idx1 * SMMU_TLB2_ENTRY_COUNT_2M) + idx2].mm_info, 0);
                }
            }
        }

        adlak_os_vfree(tlb->tlb_info);
        tlb->tlb_info = NULL;
    }
    return 0;
}

static int adlak_smmu_tlb_init(struct adlak_mem_smmu *   ptr_mm_smmu,
                               struct adlak_smmu_object *ptr_smmu, uint32_t iova_max_size_GB) {
    AML_LOG_DEBUG("%s", __func__);

    if (adlak_smmu_tlb_alloc(ptr_mm_smmu, ptr_smmu)) {
        adlak_smmu_tlb_deinit(ptr_mm_smmu, ptr_smmu);
    }
    adlak_smmu_tlb_fill_init(ptr_mm_smmu, ptr_smmu);
    return 0;
}

static int adlak_smmu_tlb_flush_cache(void *obj, struct adlak_smmu_object *ptr_smmu) {
    struct adlak_mem_smmu *          ptr_mm_smmu = obj;
...
static void adlak_smmu_unmap_iova_pages(struct adlak_smmu_object *smmu,
    ...
    for (i = 0; i < smmu_nr_pages; ++i) {
        ...
        if (mm_info->req.bytes < (1 << SMMU_TLB2_VA_SHIFT)) {
            //   adlak_hal_smmu_cache_invalid(padlak,(void *)mm->padlak, iova_addr);
        }
        iova_addr += SMMU_PAGESIZE;
    }
    if (mm_info->req.bytes >= (1 << SMMU_TLB2_VA_SHIFT)) {
        //  adlak_hal_smmu_cache_invalid(padlak,(void *)mm->padlak, 0);
    }
...
#endif
    if (ADLAK_INVALID_ADDR == mm_info->iova_addr) {
        if ((smmu->iova_byte >> 30) > 4) {
            // the smmu size more than 4GB
            if (mm_info->mem_type & ADLAK_ENUM_MEMTYPE_INNER_IOVA_WITHIN_4G) {
                mm_info->iova_addr =
...
static int adlak_smmu_cache_flush(void *obj, struct adlak_mem_handle *mm_info,
...
static int adlak_smmu_cache_invalid(void *obj, struct adlak_mem_handle *mm_info,
                                    struct adlak_sync_cache_ext_info *sync_cache_ext_info) {
    struct adlak_mem_smmu *ptr_mm_smmu = obj;
    AML_LOG_DEBUG("%s", __func__);
    if (ADLAK_IS_ERR_OR_NULL(mm_info)) {
...
static void adlak_smmu_rewrite_memtype(struct adlak_buf_req *   pbuf_req,
    ...
    if (pbuf_req->mem_type & ADLAK_ENUM_MEMTYPE_IOVA_WITHIN_4G) {
        mm_info->req.mem_type = mm_info->req.mem_type | ADLAK_ENUM_MEMTYPE_INNER_IOVA_WITHIN_4G;
    }

    mm_info->mem_type = mm_info->req.mem_type;
}

static int adlak_smmu_iova_alloc(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
                                 struct adlak_mem_handle *mm_info) {
    struct adlak_mem_smmu *   ptr_mm_smmu  = obj;
    int                       ret          = 0;
    struct adlak_smmu_object *smmu_public  = NULL;
...
static void adlak_smmu_iova_free(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
                                 struct adlak_mem_handle *mm_info) {
    struct adlak_mem_smmu *   ptr_mm_smmu  = obj;
    struct adlak_smmu_object *smmu_public  = NULL;
    struct adlak_smmu_object *ptr_smmu_pri = NULL;
...
static void adlak_smmu_free_inner(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
    ...
    AML_LOG_DEBUG("%s", __func__);
    if (has_iova) {
        adlak_smmu_iova_free(ptr_mm_smmu, ctx_smmu_attr, mm_info);
    }
...
static struct adlak_mem_handle *adlak_smmu_alloc_inner(
    ...
    mm_info->uid               = adlak_mem_uid_alloc((uintptr_t)mm_info, pbuf_req->bytes);
    ...
    if (has_iova) {
        ret = adlak_smmu_iova_alloc(ptr_mm_smmu, ctx_smmu_attr, mm_info);
        if (ret) {
            goto err_iova;
        }
    }
...
static void adlak_smmu_free(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
                            struct adlak_mem_handle *mm_info) {
    struct adlak_mem_smmu *ptr_mm_smmu = obj;
    adlak_smmu_claim_dev(ptr_mm_smmu);
    adlak_smmu_free_inner(ptr_mm_smmu, ctx_smmu_attr, mm_info, 1);
    adlak_smmu_release_dev(ptr_mm_smmu);
}

static struct adlak_mem_handle *adlak_smmu_alloc(void *                          obj,
                                                 struct adlak_context_smmu_attr *ctx_smmu_attr,
                                                 struct adlak_buf_req *          pbuf_req) {
    struct adlak_mem_smmu *  ptr_mm_smmu = obj;
    struct adlak_mem_handle *ret;
    adlak_smmu_claim_dev(ptr_mm_smmu);

    ret = adlak_smmu_alloc_inner(ptr_mm_smmu, ctx_smmu_attr, pbuf_req, 1);
    adlak_smmu_release_dev(ptr_mm_smmu);
    return ret;
}

static void adlak_smmu_dettach(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
                               struct adlak_mem_handle *mm_info) {
    struct adlak_mem_smmu *ptr_mm_smmu = obj;
    adlak_smmu_claim_dev(ptr_mm_smmu);
    adlak_smmu_iova_free(ptr_mm_smmu, ctx_smmu_attr, mm_info);
...
static struct adlak_mem_handle *adlak_smmu_attach(void *                          obj,
                                                  struct adlak_context_smmu_attr *ctx_smmu_attr,
    ...
    mm_info->uid          = adlak_mem_uid_alloc((uintptr_t)mm_info, size);
    mm_info->iova_addr    = ADLAK_INVALID_ADDR;  // init as invalid value
...
static int adlak_smmu_unmap(void *obj, struct adlak_mem_handle *mm_info) {
    // nothing todo
    return 0;
}

static int adlak_smmu_mmap(void *obj, struct adlak_mem_handle *mm_info, void *const vma) {
    struct adlak_mem_smmu *ptr_mm_smmu = obj;
    int                    ret;
    adlak_smmu_claim_dev(ptr_mm_smmu);
...
static void adlak_smmu_unmap_userspace(void *obj, struct adlak_mem_handle *mm_info) {
    struct adlak_mem_smmu *ptr_mm_smmu = obj;
    adlak_smmu_claim_dev(ptr_mm_smmu);
...
static int adlak_smmu_mmap_userspace(void *obj, struct adlak_mem_handle *mm_info) {
    struct adlak_mem_smmu *ptr_mm_smmu = obj;
    int                    ret;
    adlak_smmu_claim_dev(ptr_mm_smmu);
...
static int adlak_smmu_create_obj(struct adlak_mem_smmu *    ptr_mm_smmu,
                                 struct adlak_smmu_object **pptr_smmu, uint32_t iova_max_size_GB) {
    int                       ret      = 0;
    ...
    ret = adlak_smmu_tlb_init(ptr_mm_smmu, ptr_smmu, iova_max_size_GB);
    if (ret) {
        goto end;
    }
...
static void adlak_smmu_destroy_obj(struct adlak_mem_smmu *    ptr_mm_smmu,
                                   struct adlak_smmu_object **pptr_smmu) {
...
static void adlak_smmu_init(void *obj, void *ptr, uint8_t type) {
    struct adlak_mem_smmu *    ptr_mm_smmu = obj;
    ...
    if (ADLAK_ENUM_SMMU_TLB_TYPE_PUBLIC_ONLY == type) {
        *pptr_smmu = ptr_mm_smmu->smmu_public;
        ...
        ret = adlak_smmu_create_obj(ptr_mm_smmu, pptr_smmu, ptr_mm_smmu->padlak->iova_max_size_GB);
        if (!ret) {
...
static void adlak_smmu_deinit(void *obj, void *ptr, uint8_t type) {
    struct adlak_mem_smmu *    ptr_mm_smmu = obj;
    struct adlak_smmu_object **pptr_smmu   = ptr;
    ...
    if (ADLAK_ENUM_SMMU_TLB_TYPE_PUBLIC_ONLY != type) {
        ...
        list_for_each_entry_safe(ptr_smmu, next, &ptr_mm_smmu->smmu_list, head) {
            if (ptr_smmu == *pptr_smmu) {
                ...
                adlak_smmu_destroy_obj(ptr_mm_smmu, &ptr_smmu);
            }
        }
        adlak_smmu_release_dev(ptr_mm_smmu);
        *pptr_smmu = NULL;
...
static void adlak_smmu_get_usage(void *obj, struct adlak_mem_usage *usage) {
    struct adlak_mem_smmu *ptr_mm_smmu = obj;
    AML_LOG_DEBUG("%s", __func__);
    adlak_smmu_claim_dev(ptr_mm_smmu);
...
static uint64_t adlak_smmu_get_smmu_entry(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
                                          uint8_t type) {
    struct adlak_mem_smmu *   ptr_mm_smmu = obj;
    struct adlak_smmu_object *ptr_smmu    = NULL;
...
static void adlak_smmu_invalidate(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr) {
    struct adlak_mem_smmu *   ptr_mm_smmu  = obj;
    struct adlak_smmu_object *smmu_public  = NULL;
    struct adlak_smmu_object *ptr_smmu_pri = NULL;
    ...
    if (ctx_smmu_attr->smmu_tlb_updated) {
        ...
        adlak_hal_wait_device_idle(ptr_mm_smmu->padlak);
        if (ptr_smmu_pri) {
            adlak_smmu_tlb_flush_cache(ptr_mm_smmu, ptr_smmu_pri);
        }
        adlak_smmu_tlb_flush_cache(ptr_mm_smmu, smmu_public);
    }
    adlak_smmu_release_dev(ptr_mm_smmu);
}

static void adlak_mem_smmu_deinit(struct adlak_mem_smmu *ptr_mm_smmu) {
    struct adlak_smmu_object *ptr_smmu = NULL, *next = NULL;
    AML_LOG_DEBUG("%s", __func__);
    adlak_smmu_claim_dev(ptr_mm_smmu);
    list_for_each_entry_safe(ptr_smmu, next, &ptr_mm_smmu->smmu_list, head) {
...
static int adlak_mem_smmu_init(struct adlak_mem_smmu *ptr_mm_smmu) {
    int                       ret;
    struct adlak_mem_exclude *exclude_area = &ptr_mm_smmu->exclude_area;
    struct adlak_device *     padlak       = ptr_mm_smmu->padlak;
    AML_LOG_DEBUG("%s for public", __func__);
    ...
    ret = adlak_smmu_create_obj(ptr_mm_smmu, &ptr_mm_smmu->smmu_public,
...
void adlak_mem_smmu_unregister(struct adlak_device *padlak, void **priv,
                               struct adlak_mem_operator *ops) {
    struct adlak_mem_smmu *ptr_mm_smmu = NULL;
    AML_LOG_INFO("%s", __func__);
    if (priv && !*priv) {
        return;
    }
    ptr_mm_smmu = *priv;
    if (ptr_mm_smmu) {
        adlak_mem_smmu_deinit(ptr_mm_smmu);
...
int adlak_mem_smmu_register(struct adlak_device *padlak, void **priv,
                            struct adlak_mem_operator *ops) {
    struct adlak_mem_smmu *ptr_mm_smmu = NULL;
    int                    ret         = 0;
    if (!priv) {
    ...
    adlak_mem_smmu_unregister(padlak, priv, ops);
    AML_LOG_INFO("%s", __func__);
    if (!*priv) {
        *priv = adlak_os_zalloc(sizeof(struct adlak_mem_smmu), ADLAK_GFP_KERNEL);
        if (unlikely(!*priv)) {
            ret = -1;
            goto end;
        }
    }
    ptr_mm_smmu         = (struct adlak_mem_smmu *)*priv;
    ptr_mm_smmu->padlak = padlak;
    ...
    adlak_mem_smmu_init(ptr_mm_smmu);
