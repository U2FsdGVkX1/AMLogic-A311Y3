...
static int adlak_mem_pools_mgr_init(struct adlak_mem_cma *    ptr_mm_cma,
...
static void adlak_mem_pools_mgr_deinit(struct adlak_mem_cma *ptr_mm_cma) {
    adlak_bitmap_pool_deinit(&ptr_mm_cma->mem_pool->bitmap_area);
}

/*Reverse lookup of a region from the bitmap pool,
 */
static unsigned long adlak_alloc_share_buf_from_bitmap_pool_reverse(
    struct adlak_mem_cma *ptr_mm_cma, struct adlak_mem *mm, size_t size) {
...
static void adlak_free_to_bitmap_pool_reverse(struct adlak_mem_cma *ptr_mm_cma,
                                              struct adlak_mem *    mm) {
    struct adlak_mm_pool_priv *  pool      = &ptr_mm_cma->mem_pool->bitmap_area;
    struct adlak_share_swap_buf *share_buf = &ptr_mm_cma->share_buf;
    int                          bitmap_count;
...
static int adlak_malloc_from_mem_pool(struct adlak_mem_cma *ptr_mm_cma, struct adlak_mem *mm,
    ...
    if (!(mm_info->req.mem_type & ADLAK_ENUM_MEMTYPE_INNER_SHARE)) {
        ...
        start = adlak_alloc_share_buf_from_bitmap_pool_reverse(ptr_mm_cma, mm,
                                                               (size_t)mm_info->req.bytes);
    }

    if (ADLAK_IS_ERR((void *)start) || (ADLAK_INVALID_ADDR == start)) {
        mm_info->phys_addr = 0;
        return ERR(ENOMEM);
    }
...
static void adlak_free_to_mem_pool(struct adlak_mem_cma *ptr_mm_cma, struct adlak_mem *mm,
                                   struct adlak_mem_handle *mm_info) {
    unsigned long start;
    AML_LOG_DEBUG("%s", __func__);
    if (mm_info->phys_addr) {
        ...
        if (!(mm_info->mem_type & ADLAK_ENUM_MEMTYPE_INNER_SHARE)) {
            ...
            adlak_free_to_bitmap_pool_reverse(ptr_mm_cma, mm);
        }
    }
    mm_info->phys_addr = 0;
}
...
static void adlak_free_cma_region_nocache(struct adlak_mem_cma *ptr_mm_cma) {
...
static int adlak_alloc_cma_region_nocache(struct adlak_mem_cma *ptr_mm_cma) {
    int ret;
...
err:
    adlak_free_cma_region_nocache(ptr_mm_cma);
    return (ERR(ENOMEM));
}
#endif
...
static void adlak_free_share_through_dma(struct adlak_mem_cma *ptr_mm_cma, struct adlak_mem *mm,
                                         struct adlak_mem_handle *mm_info) {
    if (mm_info->cpu_addr) {
...
static int adlak_malloc_share_through_dma(struct adlak_mem_cma *ptr_mm_cma, struct adlak_mem *mm,
                                          struct adlak_mem_handle *mm_info) {
    int ret;
    // gfp_t gfp = 0;
...
static void inline adlak_cma_claim_dev(struct adlak_mem_cma *ptr_mm_cma) {
    AML_LOG_DEBUG("%s[+]", __func__);
    adlak_os_mutex_lock(&ptr_mm_cma->mutex);
    AML_LOG_DEBUG("%s[-]", __func__);
}

static void inline adlak_cma_release_dev(struct adlak_mem_cma *ptr_mm_cma) {
    AML_LOG_DEBUG("%s[+]", __func__);
    adlak_os_mutex_unlock(&ptr_mm_cma->mutex);
    AML_LOG_DEBUG("%s[-]", __func__);
}

static void adlak_cma_rewrite_memtype(void *obj, struct adlak_buf_req *pbuf_req,
                                      struct adlak_mem_handle *mm_info) {
    struct adlak_mem_cma *ptr_mm_cma = obj;
...
static int adlak_cma_cache_flush(void *obj, struct adlak_mem_handle *mm_info,
...
static int adlak_cma_cache_invalid(void *obj, struct adlak_mem_handle *mm_info,
                                   struct adlak_sync_cache_ext_info *sync_cache_ext_info) {
    struct adlak_mem_cma *ptr_mm_cma = obj;
    AML_LOG_DEBUG("%s", __func__);
    if (ADLAK_IS_ERR_OR_NULL(mm_info)) {
...
static void adlak_cma_free(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
                           struct adlak_mem_handle *mm_info) {
    struct adlak_mem_cma *ptr_mm_cma = obj;
    ASSERT(mm_info);
    AML_LOG_DEBUG("%s", __func__);
    adlak_cma_claim_dev(ptr_mm_cma);
    ...
    if (ptr_mm_cma->has_mem_pool) {
        ...
        if (mm_info->from_pool) {
            adlak_free_to_mem_pool(ptr_mm_cma, &ptr_mm_cma->mm, mm_info);
        } else {
            adlak_free_through_dma(&ptr_mm_cma->mm, mm_info);
        }
    } else {
        if (mm_info->mem_type & ADLAK_ENUM_MEMTYPE_INNER_SHARE) {
            adlak_free_share_through_dma(ptr_mm_cma, &ptr_mm_cma->mm, mm_info);
        } else {
    ...
    adlak_cma_release_dev(ptr_mm_cma);
}

static struct adlak_mem_handle *adlak_cma_alloc(void *                          obj,
    ...
    adlak_cma_rewrite_memtype(ptr_mm_cma, pbuf_req, mm_info);
    ...
    if (ptr_mm_cma->has_mem_pool) {
        ...
        ret = adlak_malloc_from_mem_pool(ptr_mm_cma, &ptr_mm_cma->mm, mm_info);
        if (ERR(NONE) != ret) {
            if (mm_info->req.mem_type & ADLAK_ENUM_MEMTYPE_INNER_SHARE) {
                goto end;
            }
        ...
        if (mm_info->req.mem_type & ADLAK_ENUM_MEMTYPE_INNER_SHARE) {
            ret = adlak_malloc_share_through_dma(ptr_mm_cma, &ptr_mm_cma->mm, mm_info);
            goto end;
        } else {
            // alloc memory through dma api
            ret = adlak_malloc_through_dma(&ptr_mm_cma->mm, mm_info);
...
static void adlak_cma_dettach(void *obj, struct adlak_context_smmu_attr *ctx_smmu_attr,
                              struct adlak_mem_handle *mm_info) {
    struct adlak_mem_cma *ptr_mm_cma = obj;
    AML_LOG_DEBUG("%s", __func__);
    adlak_cma_claim_dev(ptr_mm_cma);
...
static struct adlak_mem_handle *adlak_cma_attach(void *                          obj,
                                                 struct adlak_context_smmu_attr *ctx_smmu_attr,
...
static int adlak_cma_unmap(void *obj, struct adlak_mem_handle *mm_info) {
    // nothing todo
    return 0;
}

static int adlak_cma_mmap(void *obj, struct adlak_mem_handle *mm_info, void *const vma) {
    struct adlak_mem_cma *ptr_mm_cma = obj;
    int                   ret;
    AML_LOG_DEBUG("%s", __func__);
    adlak_cma_claim_dev(ptr_mm_cma);
...
static void adlak_cma_unmap_userspace(void *obj, struct adlak_mem_handle *mm_info) {
    struct adlak_mem_cma *ptr_mm_cma = obj;
    AML_LOG_DEBUG("%s", __func__);
    adlak_cma_claim_dev(ptr_mm_cma);
...
static int adlak_cma_mmap_userspace(void *obj, struct adlak_mem_handle *mm_info) {
    struct adlak_mem_cma *ptr_mm_cma = obj;
    int                   ret;
    AML_LOG_DEBUG("%s", __func__);
    adlak_cma_claim_dev(ptr_mm_cma);
...
static void adlak_cma_get_usage(void *obj, struct adlak_mem_usage *usage) {
    struct adlak_mem_cma *ptr_mm_cma = obj;
    AML_LOG_DEBUG("%s", __func__);
    adlak_cma_claim_dev(ptr_mm_cma);
...
static void adlak_mem_cma_deinit(struct adlak_mem_cma *ptr_mm_cma) {
#if CONFIG_ADLAK_MEM_POOL_EN
    if (ptr_mm_cma->has_mem_pool) {
        adlak_mem_pools_mgr_deinit(ptr_mm_cma);
...
static int adlak_mem_cma_init(struct adlak_mem_cma *ptr_mm_cma) {
    int                      ret          = 0;
...
#error "Not support CMA in freertos.Please change with reserved_memory or others"
    ...
    if (0 != adlak_alloc_cma_region_nocache(ptr_mm_cma)) {
        ret = -1;
        goto err;
    }
#endif
    ...
    if (ptr_mm_cma->has_mem_pool) {
        ...
        if (0 != adlak_mem_pools_mgr_init(ptr_mm_cma, &exclude_area)) {
            goto err;
        }
    }
...
void adlak_mem_cma_unregister(struct adlak_device *padlak, void **priv,
                              struct adlak_mem_operator *ops) {
    struct adlak_mem_cma *ptr_mm_cma = (struct adlak_mem_cma *)*priv;
    AML_LOG_INFO("%s", __func__);
    if (ptr_mm_cma) {
        adlak_mem_cma_deinit(ptr_mm_cma);
...
int adlak_mem_cma_register(struct adlak_device *padlak, void **priv,
                           struct adlak_mem_operator *ops) {
    struct adlak_mem_cma *ptr_mm_cma = NULL;
    int                   ret        = 0;
    if (!priv) {
        ret = ERR(ENODEV);
        goto end;
    }
    adlak_mem_cma_unregister(padlak, priv, ops);
    AML_LOG_INFO("%s", __func__);
    if (!*priv) {
        *priv = adlak_os_zalloc(sizeof(struct adlak_mem_cma), ADLAK_GFP_KERNEL);
        if (unlikely(!*priv)) {
            ret = ERR(ENOMEM);
            goto end;
        }
    }
    ptr_mm_cma = (struct adlak_mem_cma *)*priv;
