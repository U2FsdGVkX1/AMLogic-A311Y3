...
int adlak_mem_init(struct adlak_device *padlak) {
    int                   ret = -1;
    struct adlak_mem_ctx *mem;

    AML_LOG_INFO("%s", __func__);
    if (!padlak->mem_ctx) {
        padlak->mem_ctx = adlak_os_zalloc(sizeof(struct adlak_mem_ctx), ADLAK_GFP_KERNEL);
        if (unlikely(!padlak->mem_ctx)) {
            ret = ERR(ENOMEM);
            return ret;
        }
    }
    ...
    if (mem->use_smmu) {
#ifndef CONFIG_ADLA_FREERTOS
        ret = adlak_mem_smmu_register(padlak, &mem->priv, &mem->ops);
#endif
    } else

    {
        if (mem->use_mbd) {
            ret = adlak_mem_mbd_register(padlak, &mem->priv, &mem->ops);
        } else {
            ret = adlak_mem_cma_register(padlak, &mem->priv, &mem->ops);
        }
    }
    if (ret) {
    ...
    if (!padlak->public_cmq_mem) {
        padlak->public_cmq_mem = adlak_cmq_buf_alloc(padlak, NULL, 4096);
        if (NULL == padlak->public_cmq_mem) {
            AML_LOG_ERR("init public cmq fail!");
            return ret;
        }
    }
...
void adlak_mem_deinit(struct adlak_device *padlak) {
    struct adlak_mem_ctx *mem = padlak->mem_ctx;

    AML_LOG_DEBUG("%s", __func__);
    if (padlak->public_cmq_mem) {
        adlak_cmq_buf_free(padlak, NULL, padlak->public_cmq_mem);
        padlak->public_cmq_mem = NULL;
    }
    if (mem->use_smmu) {
#ifndef CONFIG_ADLA_FREERTOS
        adlak_mem_smmu_unregister(padlak, &mem->priv, &mem->ops);
#endif
    } else {
        if (mem->use_mbd) {
            adlak_mem_mbd_unregister(padlak, &mem->priv, &mem->ops);
        } else {
            adlak_mem_cma_unregister(padlak, &mem->priv, &mem->ops);
        }
    }
...
int adlak_mem_alloc_request(struct adlak_context *context, struct adlak_buf_req *pbuf_req) {
    ...
    // step2: alloc_usermap
    if (pbuf_req->mmap_en) {
        ret = mem->ops.mmap_userspace(mem->priv, mm_info);
        if (ret) {
...
err_attach:
    ...
    mem->ops.unmap_userspace(mem->priv, mm_info);
err_mmap:
    mem->ops.free(mem->priv, &context->smmu_attr, mm_info);
err_malloc:
    return ret;
}
...
int adlak_ext_mem_attach_request(struct adlak_context *        context,
    ...
    if (0 == pbuf_req->buf_type)  // physical addr type
    {
        ...
        mm_info = mem->ops.attach(mem->priv, &context->smmu_attr, pbuf_req);
    ...
    return 0;
err_mmap:
    mem->ops.dettach(mem->priv, &context->smmu_attr, mm_info);
err:
        ...
        list_for_each_entry_safe(pcontext, pcontext_tmp, &context->sbuf_list, head) {
            if (pcontext) {
                ...
                if (pmm_info_hd->mem_src != ADLAK_ENUM_MEMSRC_EXT_PHYS) {
                    ...
                    mem->ops.free(mem->priv, &context->smmu_attr, pmm_info_hd);
                } else {
                    mem->ops.dettach(mem->priv, &context->smmu_attr, pmm_info_hd);
                }
...
struct adlak_mem_handle *adlak_cmq_buf_alloc(struct adlak_device * padlak,
                                             struct adlak_context *context, size_t size) {
    ...
    if (context) {
        cmq_mm_info = mem->ops.alloc(mem->priv, &context->smmu_attr, &buf_req);
    } else {
        cmq_mm_info = mem->ops.alloc(mem->priv, NULL, &buf_req);
    }
    if (NULL == cmq_mm_info) {
...
void adlak_cmq_buf_free(struct adlak_device *padlak, struct adlak_context *context,
                        struct adlak_mem_handle *cmq_mm_info) {
    struct adlak_mem_ctx *mem = padlak->mem_ctx;
    if (NULL != cmq_mm_info) {
        if (context) {
            mem->ops.free(mem->priv, &context->smmu_attr, cmq_mm_info);
        } else {
            mem->ops.free(mem->priv, NULL, cmq_mm_info);
        }
    }
}
...
int adlak_mem_mmap(struct adlak_context *context, void *const vma, uint64_t uid) {
    ...
    ret = mem->ops.mmap(mem->priv, pmm_info_hd, vma);
    if (ret) {
...
int adlak_mem_get_usage(struct adlak_device *padlak, struct adlak_mem_usage *usage) {
    struct adlak_mem_ctx *mem = padlak->mem_ctx;
    mem->ops.get_usage(mem->priv, usage);
    return 0;
}

void adlak_mem_create_smmu(struct adlak_device *padlak, void *pptr_smmu, uint8_t type) {
    struct adlak_mem_ctx *mem = padlak->mem_ctx;
    if (mem->use_smmu) {
        mem->ops.smmu_init(mem->priv, pptr_smmu, type);
    }
}

void adlak_mem_destroy_smmu(struct adlak_device *padlak, void *pptr_smmu, uint8_t type) {
    struct adlak_mem_ctx *mem = padlak->mem_ctx;
    if (mem->use_smmu) {
        mem->ops.smmu_deinit(mem->priv, pptr_smmu, type);
    }
}
...
void adlak_mem_smmu_tlb_invalidate(struct adlak_context *context) {
    struct adlak_mem_ctx *mem = context->padlak->mem_ctx;
    if (mem->use_smmu) {
        mem->ops.smmu_tlb_invalidate(mem->priv, &context->smmu_attr);
    }
}
...
void adlak_mem_debug_smmu_tlb_dump(struct adlak_context *context) {
    ...
    if (mem->use_smmu) {
        if (context) {
            return mem->ops.smmu_tlb_dump(mem->priv, &context->smmu_attr);
        }
    }
}
...
int adlak_flush_cache(struct adlak_device *padlak, struct adlak_mem_handle *mm_info,
    ...
    struct adlak_mem_ctx *mem = padlak->mem_ctx;
    return mem->ops.cache_flush(mem->priv, mm_info, sync_cache_ext_info);
}

int adlak_invalid_cache(struct adlak_device *padlak, struct adlak_mem_handle *mm_info,
                        struct adlak_sync_cache_ext_info *sync_cache_ext_info) {
    struct adlak_mem_ctx *mem = padlak->mem_ctx;
    return mem->ops.cache_invalid(mem->priv, mm_info, sync_cache_ext_info);
}

void *adlak_mem_vmap(struct adlak_device *padlak, struct adlak_mem_handle *mm_info) {
    struct adlak_mem_ctx *mem = padlak->mem_ctx;
...
void adlak_mem_usage_update(struct adlak_device *padlak) {
    ...
    uapi_caps->use_smmu = 0;
    if (mem->use_smmu) {
        uapi_caps->use_smmu = 1;
...
uint64_t adlak_mem_get_smmu_entry(struct adlak_context *context, uint8_t type) {
    ...
    if (mem->use_smmu) {
        if (NULL == context) {
            return mem->ops.get_smmu_entry(mem->priv, NULL, type);
        } else {
            return mem->ops.get_smmu_entry(mem->priv, &context->smmu_attr, type);
        }
    } else {
