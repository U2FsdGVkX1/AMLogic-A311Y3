...
/**
 * @brief Simple hash algorithm to generate a UID.
 */
static uint32_t generate_uid(uint64_t a, uint64_t b) {
    uint64_t hash = a ^ (b + 0x9e3779b97f4a7c15ULL + (a << 6) + (a >> 2));
    hash ^= hash >> 33;
...
uint64_t adlak_mem_uid_alloc(uintptr_t val, uint64_t size) {
    uint64_t uid;
    uid = generate_uid(val, size) << ADLAK_MM_POOL_PAGE_SHIFT;
    return uid;
}
